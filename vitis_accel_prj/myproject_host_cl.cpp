...
void runFPGAHelper(FpgaObj<float, float> &fpga) { fpga.runFPGA(); }
...
int main(int argc, char **argv) {
	...
	HbmFpga<float, float> fpga(BATCHSIZE * INSTREAMSIZE, BATCHSIZE * OUTSTREAMSIZE, NUM_CU, NUM_THREAD, 10);
    ...
    if (fin.is_open()) {
        ...
        while (std::getline(fin, iline)) {
            ...
            std::string token;
            while (in >> token) {
                inputData.push_back(stof(token));
            }
            num_inputs++;
        }
    }
    ...
    memcpy(fpga.source_in.data(), inputData.data(), num_samples * INSTREAMSIZE * sizeof(float));
    ...
    if (resultsFile.is_open()) {
        for (int i = 0; i < num_samples; i++) {
            ...
            for (int n = 0; n < DATA_SIZE_OUT; n++) {
                oline << fpga.source_hw_results[(i * DATA_SIZE_OUT) + n] << " ";
            }
